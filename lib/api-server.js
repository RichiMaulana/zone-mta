...
const pluginsRestify = require('restify-plugins')
...
class APIServer {
    ...
    createServer() {
        ...
        this.server.use(pluginsRestify.multipartBodyParser())
        this.server.use(
    ...
    setupRoutes() {
        ...
        this.server.post('/send', upload.single('attachment'), (req, res, next) => {
            let originalname, mimetype, buffer, size;
            if (req.file) {
            ...
            if (!authHeader || !authHeader.startsWith('Bearer ')) {
                res.json(401, { status: 'Failed', code: 401, msg: 'No token provided!' });
                return next();
            }
            ...
            const options = { projection: { companyUuid: 1, token: 1 } };
            ...
            tokenCollection.findOne({ token: authHeader }, options, (err, token) => {
                ...
                listdomainCollection.find({ companyUuid: token.companyUuid }).toArray((err, domains) => {
                    if (err) {
                        res.json(500, { error: 'Internal server error' });
                        return next();
                    }

                    domains = domains.map(el => el.domain);
                    const senderDomain = data.from.split('@')[1];
                    if (!domains.includes(senderDomain)) {
                        res.json(401, { error: 'Relay access denied' });
                        return next();
                    }
