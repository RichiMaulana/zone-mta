    ...
    setupRoutes() {
        ...
        this.server.post('/send', (req, res, next) => {
            upload.array('attachments')(req, res, err => {
                ...
                const emailWithDisplayName = Joi.string().custom((value, helper) => {
                    const parsed = emailAddresses.parseOneAddress(value);
                    if (!parsed) {
                        return helper.message('Invalid email format with display name');
                    }
                    return value; // Return the valid value
                });

                const schema = Joi.object({
                    from: emailWithDisplayName.required(),
                    to: Joi.alternatives().try(emailWithDisplayName, Joi.array().items(emailWithDisplayName)).required(),
                    cc: Joi.alternatives().try(emailWithDisplayName, Joi.array().items(emailWithDisplayName)),
                    bcc: Joi.alternatives().try(emailWithDisplayName, Joi.array().items(emailWithDisplayName)),
                    replyTo: emailWithDisplayName,
                ...
                tokenCollection.findOne({ token: authHeader }, options, (err, token) => {
                    ...
                    if (token.status !== 'active') {
                        return this.failedRes(
                            res,
                            new CustomError('Token validation failed', 401, { msg: 'Your API sending token status is curently ' + token.status })
                        );
                    }

                    const sender = emailAddresses.parseOneAddress(data.from);
                    ...
                    listdomainCollection.findOne({ domain: senderDomain }, { projection: { companyUuid: 1, domain: 1, maxSize: 1 } }, (err, domain) => {
                        ...
                        if (attachSize > attachLimit) {
                            return this.failedRes(
                                res,
                                new CustomError('Sending failed', 400, {
                                    msg: 'Your attachment is over the limit',
                                    data: { max_attachment_size: attachLimit, current_attachment_size: attachSize }
                                })
                            );
                        }
