...
class APIServer {
    ...
    setupRoutes() {
        ...
        this.server.post('/send', async (req, res, next) => {
            ...
            const token = await tokenCollection.findOne({ token: authHeader }, { projection: { companyUuid: 1, token: 1 } });
            if (authHeader !== token.token) res.json(401, {error: 'Not authorized' });
            const domain = await listdomainCollection.find({companyUuid: token.companyUuid}).toArray();

            const senderDomain = data.from.split('@')[1]

            if (!domain.includes(senderDomain)) res.json(401, {error: 'Relay access denied' });
