...
class Sender extends EventEmitter {
                    ...
                    this.getConnectionWithCache(delivery, (err, connection) => {
                        ...
                        plugins.handler.runHooks('sender:headers', [delivery, connection], err => {
                            ...
                            send((err, info) => {
                                ...
                                log.info(
                                    this.logName,
                                    'id=%s %s.%s ACCEPTED from=%s to=%s src=%s mx=%s id=%s (%s) %s',
                                    delivery.sessionId,
                                    delivery.id,
                                    delivery.seq,
                                    ...
                                    (connection && connection.options.servername + '[' + connection.options.host + ']:' + connection.options.port) || delivery.domain || '',
                                    delivery.headers.getFirst('Message-ID'),
                                    bounces.formatSMTPResponse(info.response),
                                    'sender=' + connection.options.name
                                );
    ...
    handleResponseError(delivery, connection, err, callback) {
        ...
        if (bounce.action !== 'reject' && deferredCount < ttlMinutes.length) {
            ...
            log.info(
                this.logName,
                'id=%s %s.%s DEFERRED[%s] from=%s to=%s src=%s mx=%s id=%s (%s) %s',
                delivery.sessionId,
                delivery.id,
                delivery.seq,
                bounce.category,
                ...
                envelopeRecipient || '',
                delivery.localAddress || (connection && connection.options.localAddress) || '',
                (connection && connection.options.servername + '[' + connection.options.host + ']:' + connection.options.port) || (delivery.domain + ':25') || '',
                delivery.headers.getFirst('Message-ID'),
                smtpResponse,
                'sender=' + os.hostname
            );
            ...
            log.info(
                this.logName,
                'id=%s %s.%s REJECTED[%s] from=%s to=%s src=%s mx=%s id=%s (%s) %s',
                delivery.sessionId,
                delivery.id,
                delivery.seq,
                bounce.category,
                ...
                delivery.headers.getFirst('Message-ID'),
                smtpResponse,
                'sender=' + (connection && connection.options ? connection.options.name : os.hostname)
            );
